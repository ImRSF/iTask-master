...
class SingleFlow extends Binder {
  constructor(props) {
    ...
    this._bind("_handleFormChange", "_handleTaskSubmit", "getTaskCheckbox");
  }
  ...
  getTaskCheckbox(selectedTask, isComplete) {
    let component;
  ...
  render() {
    ...
    const { defaultTask, flowStore, match, taskStore, loggedInUser } =
      this.props;
                ...
                <ul style={{ listStyleType: "none" }}>
                  {taskListItems.map((task, i) => {
                    if (!task.complete) {
                      return (
                        <li key={task._id + i}>
                          {this.getTaskCheckbox(task, false)}
                          {/* <CheckboxInput
                            label={task.name}
                            value={false}
                            change={this._handleFormChange}
                ...
                <ul style={{ listStyleType: "none" }}>
                  {taskListItems.map((task, i) => {
                    if (task.complete) {
                      return (
                        <li key={task._id + i}>
                          {this.getTaskCheckbox(task, true)}
                          <Link
                            className="yt-btn x-small bordered info"
                            to={`/tasks/${task._id}`}
                          >
